        ...
        return tokens.tokens
          ...
          .map((token, index) => (
            <div className='tokenItem' key={index}>
              ...
              <Link href={`/token/${token.contractAddress}`}> {/* Link to the token's page */}
                {token.name} ({token.symbol})<span className='emojiButton emoji'><Image alt={token.name} width="24" height="24" src={token.emoji} unoptimized/></span>
              </Link>
            </div>
          ));
      };
